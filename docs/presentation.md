...
ctl-core's direct Serde and MiniJinja edges are isolated behind
`surface-templates`. `surface-model` still inherits Serde from `usage-lib`,
whose KDL model requires it. The existing `surface` feature remains the
compatibility aggregate enabling both.
